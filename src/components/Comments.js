...
export function Comments({id, uid, comments}) {
    ...
    return (
        <Bounded>
            ...
            {comments.length > 0 && (
                <>
                    <Heading as="h3" size="3xl" className="mt-12">
                        What people are saying
                    </Heading>
                    {comments.map((comment) => (
                        <div className="p-6 border my-4" key={comment.id}>
                            <header className="text-sm">
                                {`Posted by ${comment.nickname} on ${new Date(comment.created_at).toLocaleTimeString("pt-br", {
                                    year: "numeric",
                                    month: "short",
                                    day: "numeric",
                                })}`}
            ...
            <form onSubmit={onSubmit}>
                ...
                <button type="submit" className="p-4 bg-slate-700 text-white mt-6" disabled={loading}>
